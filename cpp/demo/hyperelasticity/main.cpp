...
public:
  HyperElasticProblem(
      ...
      : _u(u), _l(L), _j(J), _bcs(bcs),
        _b(L->function_spaces()[0]->dofmap()->index_map,
           L->function_spaces()[0]->element()->block_size()),
        _matA(fem::create_matrix(*J))
  {
    auto map = L->function_spaces()[0]->dofmap()->index_map;
    const int bs = L->function_spaces()[0]->element()->block_size();
  ...
  Mat J(const Vec) final
  {
    ...
    fem::assemble_matrix(la::PETScMatrix::add_fn_block(_matA.mat()), *_j, _bcs);
